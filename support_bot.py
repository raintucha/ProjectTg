...
async def show_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # --- Текст помощи для жителей и покупателей ---
        if role == SUPPORT_ROLES["user"]:
            help_text = (
                "ℹ️ **Справочный центр**\n\n"
                "Здравствуйте! Я бот-помощник ЖК Sunqar. Вот как мной пользоваться:\n\n"
                "**Для жителей:**\n"
                "📝 **Оставить заявку** — Нажмите, чтобы сообщить о проблеме. Вы можете написать текст, а также прикрепить фото, видео или голосовое сообщение.\n\n"
                "📂 **Мои заявки** — Здесь можно посмотреть историю всех ваших обращений и узнать их статус (например, 'в работе' или 'выполнено').\n\n"
                # ЗАВТРА ДОБАВИТЕ ССЫЛКУ СЮДА
                # "🎬 **Видеоинструкция:** [нажмите здесь, чтобы посмотреть](ссылка_на_ваше_видео)\n\n"
                f"• Новости ЖК: {NEWS_CHANNEL}\n"
                f"• Техподдержка: @ShiroOni99"
            )
        # --- Текст помощи для персонала ---
        else:
            help_text = (
                "ℹ️ **Справка для персонала**\n\n"
                "Эта секция поможет вам в работе с заявками:\n\n"
                "🔔 **Активные заявки** — Здесь находятся все новые обращения от жителей, которые требуют вашего внимания.\n\n"
                "🚨 **Срочные** — Отфильтрованный список только срочных заявок.\n\n"
                "✅ **Завершенные** — Архив всех выполненных заявок.\n\n"
                "🔍 **Смотреть заявку** — При нажатии на эту кнопку вы увидите все детали обращения, а если к нему прикреплены фото или видео, они придут следующим сообщением."
            )

        # Создаем отдельную клавиатуру только с кнопкой "Назад"
        keyboard = [
        ...
        await send_and_remember(
            ...
            disable_web_page_preview=True # Эта опция уберет большое превью, если добавите ссылку
        )
    ...
    try:
        ...
        if query.data == "req_prev":
            ...
            if current_role == SUPPORT_ROLES["admin"]:
                ...
                welcome_text = "Здравствуйте! Я бот-помощник ЖК Sunqar. Помогу оставить заявку или узнать о комплексе. Выберите, что вас интересует 👇"
            await send_and_remember(
                update,
                context,
                welcome_text,
